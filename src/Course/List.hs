...
filter ::
  ...
  -> List a
filter _ Nil = Nil
filter func (h :. l) = if (func h) == True 
                       then h :. filter func l
                       else filter func l
...
seqOptional = foldRight f (Full Nil) where
  f Empty _ = Empty
  f _ Empty = Empty
  f (Full kek) (Full lol) = Full (kek :. lol)
...
find ::
  ...
  -> Optional a
find _ Nil = Empty
find f (h :. l) = if f h then Full h else find f l
...
notReverse ::
  List a
  -> List a
notReverse l = reverse . reverse $ l
